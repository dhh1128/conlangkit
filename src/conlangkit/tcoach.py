...
SimpleNormalizationRule.apply = lambda self, text: text.replace(  # type: ignore[attr-defined]
    self.pattern, self.replacement
)


def snr(pattern, replacement):
    return SimpleNormalizationRule(pattern, replacement)
