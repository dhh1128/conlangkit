from ..phoneme import *
from ..phoneme import _n2v, _ph, _v2n
...
def test_attrib_bits_to_phrase_round_trip():
    attrib_phrase_to_bits("voiced bilabial plosive")
    # print(show_relevant_bits(y))
    attrib_bits_to_phrase(BILABIAL | PLOSIVE | VOICED)
    for p in _ph:
        expected = p[2]
...
def check_2(func, should_be_true, should_be_false):
    assert func(ByIPA[should_be_true])
    assert not func(ByIPA[should_be_false])
...
def test_can_be_vocalic():
    ...
    assert not ByIPA["s"].can_be_vocalic
