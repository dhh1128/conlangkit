...
_ph = [
    ...
    (
        ...
        'English speak [spik], French pose [poz], Spanish perro ["pero]',
    ),
    ...
    (
        ...
        "Scots loch [lOx] or [5Ox]; German Buch, Dach; Spanish caja, gestión",
    ),
    ...
    (
        ...
        'German ich [IC], English human ["Cjum@n] (broad transcription uses [hj-])',
    ),
    ...
    (
        ...
        "French même [mE:m], English met [mEt] (RP and Gen.Am.)",
    ),
    (
        "F",
        "ɱ",
    ...
    (
        ...
        "Welsh tu [t1], American English rose's [\"r\\oUz1z]",
    ),
    ...
    (
        ...
        'German besser ["bEs6], Australian English mud [m6d]',
    ),
...
class Phoneme:
    ...
    def sonority(self) -> float:
        if self.is_vowel:
            return VOWEL_SONORITY
        if self.is_glide:
            return GLIDE_SONORITY
        if self.is_liquid:
            return LIQUID_SONORITY
        if self.is_nasal:
            return NASAL_SONORITY
        if self.is_fricative:
            n = FRICATIVE_SONORITY
        elif self.is_affricate:
            n = AFFRICATE_SONORITY
        elif self.is_plosive:
            n = PLOSIVE_SONORITY
        else:
            n = NON_PULMONIC_SONORITY
        if self.is_voiced:
