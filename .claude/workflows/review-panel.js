export const meta = {
  name: 'conlangkit-review-panel',
  description: 'Multi-persona adversarial code review of the conlangkit repo; dedupes findings by dedupe_key and adjudicates dispositions. Self-contained — persona prompts are vendored under prompts/review/. args is an OBJECT (see whenToUse).',
  whenToUse: 'At a milestone, for a multi-lens review of conlangkit. Reads source read-only but WRITES its output to <target>/reviews/ (uncommitted): one synthesis file plus a per-persona report each, then returns the triaged queue + summary. ARGS (object): target = the conlangkit repo folder, relative ("." / "../conlangkit") or absolute; for a relative target also pass baseDir = the launching session absolute cwd. For "this repo"/no explicit target, the orchestrator resolves the session git root (git rev-parse --show-toplevel) and passes it as target. personas = optional array of names ("security","maintainability","testability","api","linguist","devops","architect","skeptic") or prefixes ("SEC","MNT","TST","API","LNG","OPS","ARC","SKP"); omit to run the DEFAULT five (security, maintainability, testability, api-contract, computational-linguist) and name "devops"/"architect"/"skeptic" to add them; or the string "auto" to let a git-aware Scope phase pick lenses from the diff/history. Optional: branch (abort if target is not on it), milestone (run label), concurrency (default 2), promptsDir (override the vendored prompts/review/ location). Effort/model tiering: api-contract runs deep+Opus, computational-linguist+architect deep+Sonnet, the rest medium+Sonnet; override run-wide with effort + model, or per-persona with overrides = {PREFIX: {effort, model}}. verify ("off"|"default"|"all", default "default") runs a pre-merge pass that adversarially tries to REFUTE high-stakes findings (default = api-contract/security/CRITICAL; "all" = every CRITICAL+HIGH recommend-fix); refuted findings are excluded from the queue but recorded in the report.',
  phases: [
    ...
    { title: 'Verify', detail: 'adversarially refute high-stakes findings (api-contract/security/CRITICAL) before merge; refuted findings are excluded but recorded' },
...
// ---- inputs ----
// args.target may be relative ("." / "../conlangkit") or absolute. A relative target is only
// unambiguous against an explicit base, so it requires args.baseDir = the launching session's absolute
// cwd. We never resolve a relative path against the workflow agents' ambient cwd. The preflight
// canonicalizes whatever we get to the enclosing git repo root.
if (!args || typeof args.target !== 'string' || !args.target.trim()) {
  throw new Error('review-panel requires args.target = the conlangkit repo folder (relative like "." / "../conlangkit", or absolute). For a relative target, also pass args.baseDir = the absolute directory it is relative to.')
}
...
const CONCURRENCY = (args && args.concurrency) || 2 // self-throttle for this RAM-limited machine

// Per-persona effort/model defaults. conlangkit's headline claims are a stable CONSUMER API and
// LINGUISTICALLY sound domain modeling, so api-contract runs deep on the strongest model (its
// findings can silently break every downstream consumer), computational-linguist and architect run
// deep on Sonnet, and the inherited defect lenses run cheap (medium effort, Sonnet). All OVERRIDABLE
// via args.effort / args.model / args.overrides.
const ALL_PERSONAS = [
  { file: 'security-hawk.md', prefix: 'SEC', persona: 'security-hawk', effort: 'medium', model: 'claude-sonnet-5' },
  { file: 'maintainability-expert.md', prefix: 'MNT', persona: 'maintainability-expert', effort: 'medium', model: 'claude-sonnet-5' },
  { file: 'testability-hawk.md', prefix: 'TST', persona: 'testability-hawk', effort: 'medium', model: 'claude-sonnet-5' },
  { file: 'api-contract-auditor.md', prefix: 'API', persona: 'api-contract-auditor', effort: 'deep', model: 'claude-opus-4-8' },
  { file: 'computational-linguist.md', prefix: 'LNG', persona: 'computational-linguist', effort: 'deep', model: 'claude-sonnet-5' },
  { file: 'devops-engineer.md', prefix: 'OPS', persona: 'devops-engineer', effort: 'medium', model: 'claude-sonnet-5' },
  { file: 'architect.md', prefix: 'ARC', persona: 'architect', effort: 'deep', model: 'claude-sonnet-5' },
  { file: 'first-principles-skeptic.md', prefix: 'SKP', persona: 'first-principles-skeptic', effort: 'medium', model: 'claude-sonnet-5' },
]
// Accept either prefixes (SEC, MNT, …) or friendly names.
const NAME_TO_PREFIX = {
  ...
  api: 'API', contract: 'API', 'api-contract': 'API', 'api-contract-auditor': 'API',
  lng: 'LNG', ling: 'LNG', linguist: 'LNG', linguistics: 'LNG', 'computational-linguist': 'LNG',
  ops: 'OPS', devops: 'OPS', 'devops-engineer': 'OPS',
  arc: 'ARC', architect: 'ARC', architecture: 'ARC',
  skp: 'SKP', skeptic: 'SKP', sceptic: 'SKP', 'first-principles-skeptic': 'SKP',
}
...
// Default panel when no personas are named: security, maintainability, testability, api-contract,
// computational-linguist. Name devops / architect / skeptic explicitly via args.personas to add them.
const DEFAULT_PREFIXES = ['SEC', 'MNT', 'TST', 'API', 'LNG']
...
// Persona prompts are VENDORED in the conlangkit repo (self-contained — no external clone dependency).
...
if (AUTO_SCOPE) {
  ...
  const scope = await agent(
    `TARGETING — read carefully: before anything, run \`git -C "${TARGET}" rev-parse --show-toplevel\` and confirm ` +
      ...
      `You are scoping which code-review lenses to run for this milestone of conlangkit. Inspect, under \`nice -n 19 ionice -c 3\`:\n` +
      ...
      `Decide which lenses to RUN. Reason from the actual diff/changes. A change to the public API surface ` +
      `(glossary.py, lang.py, tcoach.rewrite_rules, bfr.py, or py.typed) almost always warrants API; a change to ` +
      `the phonology/morphology/POS/orthography modeling (phoneme.py, syllable.py, bfr.py, pos.py, ortho.py, the ` +
      `glossary equivalence operators) warrants LNG; a change to the dynamic-import/plugin surface or glossary ` +
      `parsing warrants SEC; a change to .github/ warrants OPS. Recommend SKIPPING a lens ONLY when a recent review ` +
      `for it exists AND no changed file is relevant to that lens since that review's reviewed commit. Be ` +
      `CONSERVATIVE about API and SEC: skip them only when there are ZERO relevant changes since their last review. ` +
      `Every skip MUST carry a human-readable reason.\n\n` +
...
const perPersona = await runChunked(personaPlan, CONCURRENCY, (p) =>
  agent(
    `TARGETING — read carefully:\n` +
      ...
      `(mode: unattended, effort: ${p.eff}, run_label: "${milestone}"). Run heavy shell work — the test suite, mypy, ` +
      `ruff, or the CLI — under \`nice -n 19 ionice -c 3\`. ` +
...
// ---- Phase 1.5: adversarial verification — try to REFUTE high-stakes findings ----
// Gated by args.verify:
//   'off'             → skip entirely
//   'all'             → verify all CRITICAL+HIGH recommend-fix findings
//   unset | 'default' → verify api-contract / security findings OR any CRITICAL
...
const verifyGate = (f) => {
  ...
  // default
  return f.persona === 'api-contract-auditor' || f.persona === 'security-hawk' || f.severity === 'CRITICAL'
}
let refuted = []
...
if (verifyMode !== 'off') {
  ...
  if (toVerify.length === 0) {
    ...
    const verifyOne = (f) => {
      // CAPABLE verifier (Opus, full agent) for api-contract, security, or any CRITICAL; LIGHT read-only Explore otherwise.
      const capable = f.persona === 'api-contract-auditor' || f.persona === 'security-hawk' || f.severity === 'CRITICAL'
      ...
      return agent(
        `TARGETING — read carefully: before doing anything, run \`git -C "${TARGET}" rev-parse --show-toplevel\` ` +
          ...
          `Review ONLY the repository at "${TARGET}". Run heavy shell work (including running the tests or the CLI) under \`nice -n 19 ionice -c 3\`.\n\n` +
          `Your job is to ADVERSARIALLY attempt to REFUTE the following code-review finding by inspecting the actual code, the docstrings/type annotations, and — where it settles the question — the tests or the CLI behavior. ` +
          `Do NOT try to confirm it; actively look for evidence that it is wrong, already mitigated, a false positive, or not present. For an api-contract claim, confirm the cited public symbol/signature/docstring actually exists and that the behavior truly diverges.\n` +
          `Finding:\n` +
          `  id: ${f.id}\n` +
          `  title: ${f.title}\n` +
          `  location: ${f.location}\n` +
...
const synth = await agent(
  `You are the review orchestrator for the conlangkit repo at ${TARGET} (branch ${reviewedBranch}), milestone "${milestone}". ` +
    ...
    `the SAME underlying issue (e.g. an api-contract finding and a maintainability finding both about a ` +
    `docstring that contradicts the code). Judge sameness from each finding's title + location + rationale. Return ` +
    `an array of arrays of finding "id" strings, where each inner array is a set of ids describing the SAME ` +
    `underlying issue. Be CONSERVATIVE: only group findings that are genuinely the same issue. Singletons need not ` +
    `be listed; return [] if nothing should merge.`,
